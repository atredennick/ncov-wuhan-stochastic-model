...
params <- list(beta0 = 0.657, beta.factor = 1, sigma = 1/6.4, I0 = 1)
...
stat_times <- ma(diff(log(covid_sl@data+10)), n = 5)
...
```{r normality, fig.height=2.5, eval = FALSE}
test_params <- c(beta0 = 0.657, beta.factor = 1, sigma = 1/6.4, I0 = 1) 
...
```{r lik-test}
test_params <- c(beta0 = 0.5, beta.factor = 1, sigma = 1/6.4, I0 = 1) 
slik(covid_sl, param  = unname(log(test_params)), nsim = 100)
```

And we can look at the log likelihood along a slice of $\beta_0$.
```{r slice, eval = FALSE}
beta_slice <- log(seq(0.5, 0.7, by = 0.01))
...
slice(covid_sl, ranges = list("beta0" = beta_slice), nsim = 100,
      multicore = TRUE, cluster = cl, ncores = num_cores, trans = c(beta0 = "exp"))
# Stop the cluster
...
covid_priors_sl <- function(input, ...) { sum( input ) +
    dnorm (input[1], log(0.67), 0.5, log = TRUE) +  # beta0 prior
    dnorm (input[2], log(1), 0.5, log = TRUE) +  # beta reduction prior
    dnorm (input[3], log(1/6.4), 0.5, log = TRUE) +  # sigma
    dunif (input[4], 1, 100, log = TRUE) }  # initial I1
```

Finally, we can run the ```synlik::smcmc``` routine.
```{r smcmc, fig.height=2.5}

sum_stats <- list(ss1 = function(traj))

my_distance <- function(sum_stats, data_obs, model_sims) {
    # calculate the absolute distance of each summary statistic,
    # and take the mean
    res <- mean(sapply(sum_stats, function(x) {
        abs(x(model_sims) - x(data_obs))
    }))
...
simu <- 
ssMeanRelDistance(sum_stats = list(ssSize, ssMax),
                  data.obs = FluTdC1971,
                  model.obs = simu)
...
# covMat <- diag(c(0.1, 0.2, 0.2, 1))^2
# covMat <- diag(c(0.001, 0.001, 0.001, 0.001))
# 
# # Set the initial parameters, arithmetic scale
# init_params <- c(beta0 = 0.35, beta.factor = 4, sigma = 0.25, I0 = 10)
# 
...
# # Plot the chains
# par(mfrow = c(2, 2))
# plot(exp(covid_mcmc@chains[,1]), type = "l", xlab = "Iteration", 
#      ylab = expression(beta), las = 1)
# abline(h = exp(covid_sl@param[1]), col = "red")
# plot(exp(covid_mcmc@chains[,2]), type = "l", xlab = "Iteration",
#      ylab = expression(xi), las = 1)
# abline(h = exp(covid_sl@param[2]), col = "red")
# plot(exp(covid_mcmc@chains[,3]), type = "l", xlab = "Iteration",
#      ylab = expression(sigma), las = 1)
# abline(h = exp(covid_sl@param[3]), col = "red")
# plot(ceiling(exp(covid_mcmc@chains[,4])), type = "l", xlab = "Iteration",
#      ylab = expression(I[0]), las = 1)
# abline(h = exp(covid_sl@param[4]), col = "red")
